                  ...
                  {week.map((date, dayIndex) => {
                    ...
                    return (
                      <div
                        ...
                        className={`w-4 h-4 rounded-sm ${date ? getColor(level) + ' cursor-pointer' : 'bg-transparent'
                          } hover:ring-1 hover:ring-gray-400 transition-all`}
      ...
      <div className="w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-6">
          ...
          <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-6 shadow-lg">
            <h4 className="text-xl font-bold text-purple-800 mb-4">Activity Stats</h4>
            <div className="space-y-3">
              <div>
                <div className="text-2xl font-black text-purple-600">{totalActivities}</div>
                <div className="text-sm text-purple-500">Total Activities</div>
              </div>
              <div>
                <div className="text-xl font-bold text-purple-600">{data.length}</div>
                <div className="text-sm text-purple-500">Active Days</div>
              </div>
              <div>
                <div className="text-xl font-bold text-purple-600">
                  {Math.round(totalActivities / data.length)}
                </div>
                <div className="text-sm text-purple-500">Avg per Day</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-6 shadow-lg">
            <h4 className="text-lg font-bold text-gray-700 mb-4">Activity Levels</h4>
            <div className="space-y-3">
              {[
          ...
          <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 shadow-lg">
            <h4 className="text-lg font-bold text-gray-700 mb-4">Recent Activity</h4>
            <div className="space-y-2">
              {data.slice(-5).reverse().map((activity, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-semibold text-gray-700">{activity.count}</span>
                </div>
              ))}
            </div>
          </div>
